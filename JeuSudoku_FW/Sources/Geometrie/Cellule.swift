...
public struct Cellule: Testable, InstanciableParNom {
    ...
    public init?(nom: String) {
        ...
        if ligneColonne.count != 2 { return nil }
        let ligne = ligneColonne[0]
        let colonne = ligneColonne[1]
        if !Ligne.noms.contains(ligne) { return nil }
        if !Colonne.noms.contains(colonne) { return nil }
